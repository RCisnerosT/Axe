...
def test_format_divergence_alert_includes_direction_strength_scope_ticker_price_and_dates():
    text = telegram.format_divergence_alert(
        "AAPL", "1h", "extended", "bullish", "strong", 123.45, PIVOT_1_TS, PIVOT_2_TS
    )
    assert "BULLISH" in text
    assert "STRONG" in text
    assert "AAPL" in text
    assert "1h" in text
    assert "incl. pre/post" in text
...
def test_format_divergence_alert_labels_regular_session():
    text = telegram.format_divergence_alert(
        "AAPL", "1h", "regular", "bullish", "strong", 123.45, PIVOT_1_TS, PIVOT_2_TS
    )
    assert "regular session" in text


def test_format_divergence_alert_labels_weak_signals():
    text = telegram.format_divergence_alert(
        "AAPL", "1d", "regular", "bullish", "weak", 123.45, PIVOT_1_TS, PIVOT_2_TS
    )
    assert "weak" in text
...
def test_send_divergence_alert_reaches_telegram():
    telegram.send_divergence_alert("AAPL", "1d", "regular", "bullish", "strong", 123.45, PIVOT_1_TS, PIVOT_2_TS)
